...
namespace School_Management_System.Repositories.Implementations
{
    public class TeacherRepository : ITeacherRepository
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(name)) return Enumerable.Empty<Teacher>();
            return _context.Teachers
                .Include(t => t.Department)
        ...
        {
            if (string.IsNullOrWhiteSpace(role)) return Enumerable.Empty<Teacher>();
            return _context.Teachers
                .Include(t => t.Department)
        ...
        {
            return _context.Teachers.Include(t => t.Department).Include(t => t.Classes).Include(t => t.Courses)
                .FirstOrDefault(t => t.TeacherID == id) ?? throw new KeyNotFoundException($"Teacher with ID {id} not found.");
        }
